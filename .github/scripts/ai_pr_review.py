...
def main() -> None:
    ...
    # If only site files changed (and they were already generated by generate-site job), just acknowledge
    if changed_site_files and not changed_files:
        post_pr_comment(repo, pr_number, f"✅ AI review: {len(changed_site_files)} site file(s) changed (generated by AI). No category files to review.")
        return
    
    # If no files changed at all, acknowledge
    if not changed_files and not changed_site_files:
